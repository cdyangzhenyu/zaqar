...
_TRANSPORT_LIMITS_OPTIONS = (
    ...
    cfg.ListOpt('subscriber_types', default=['http', 'https', 'mailto', 'queue',
...
class Validator(object):
        ...
        if queue_delay_ttl:
            if not (MIN_MESSAGE_DELAY_TTL <= queue_delay_ttl <=
                ...
                msg = _(u'delay_ttl can not exceed {0} '
                        'seconds, and must be at least {1} seconds long.')
                raise ValidationFailed(
                    msg, self._limits_conf.max_message_delay_ttl, MIN_MESSAGE_DELAY_TTL)
        ...
        if queue_claim_ttl:
            if not (MIN_MESSAGE_CLAIM_TTL <= queue_claim_ttl <=
                ...
                msg = _(u'claim_ttl can not exceed {0} '
                        'seconds, and must be at least {1} seconds long.')
                raise ValidationFailed(
                    msg, self._limits_conf.max_claim_ttl, MIN_MESSAGE_CLAIM_TTL)
    ...
    def message_content(self, message):
        ...
        if not (MIN_MESSAGE_DELAY_TTL <= delay_ttl <= self._limits_conf.max_message_delay_ttl):
            msg = _(u'The Delay TTL for a message may not exceed {0} seconds, and '
                    'must be at least {1} seconds long.')
